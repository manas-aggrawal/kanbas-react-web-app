...
export const findAttemptsForQuizByUser = async (
  quizId: any,
  userId: string
) => {
...
export const getQuizAttemptsForUserForCourse = async (
  courseId: any,
  userId: string
) => {
  const response = await axios.post(`${QUIZZES_API}/all-attempts-for-course`, {
    courseId: courseId,
    userId: userId,
  });
  return response.data;
